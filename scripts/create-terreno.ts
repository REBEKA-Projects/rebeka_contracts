...
async function main() {
  ...
  if (!factoryAddress) {
    throw new Error("RWA_FACTORY_ADDRESS no está definido (dirección de RWATokenFactory en Arbitrum).");
  }
  ...
  console.log("Multisig (admin/issuer/kycAdmin/pauser):", multisig);
  console.log(`Creating share token: ${name} (${symbol})`);

  const factory = await viem.getContractAt("RWATokenFactory", factoryAddress, {
